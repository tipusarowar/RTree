...
BlockFile::BlockFile(char* name,int b_length)
{
   ...
   if((fp=fopen(name,"rb+"))!=0)
   {
      ...
      number = fread_number();
	  printf("Inside if((fp=fopen(name,rb+))!=0) :Data read from disk : blocklength=%d number=%d\n",blocklength,number);
   }
   else
	   
   {
	  ...
	  //printf("Sarowar debug: before put_bytes=%d\n",(int)ftell(fp));
      put_bytes(buffer,l);    //fill in the rest of block with 0

      delete [] buffer;

	  printf("Sarowar debug: Inside Else part of if((fp=fopen(name,rb+))!=0)i.e. condition is false. fp=fopen(filename,wb+); :  blocklength=%d  BFHEAD_LENGTH=%d \n (l=%d = blocklength - (int)ftell(fp)=%d) \n",blocklength,BFHEAD_LENGTH,l,(int)ftell(fp));
   }

   fseek(fp,0,SEEK_SET);
   act_block=0;		
   printf("BlockFile ends.\n");
}
