...
bool LinList::erase()
{
    ...
    if (akt)
    {
		printf("Sarowar debug: LinList::erase(): if (akt)=true.\n");
	// Element ist erstes Element
	if (akt == first)
	{	
		printf("Sarowar debug: LinList::erase(): if (akt == first)=true.\n");
	    // Element ist einziges Element
	    if (akt == last)
	    {
			printf("Sarowar debug: LinList::erase(): if (akt == last)=true.\n");
		akt_index = -1;
		first = last = NULL;
		n_akt = NULL;
	    }
	    ...
	    {
			printf("Sarowar debug: LinList::erase(): if (akt == last)=false.\n");
	...
	{
		printf("Sarowar debug: LinList::erase(): if (akt == first)=false.\n");
...
void LinList::print()
{
    ...
    while (cur != NULL)
    {
		//Sarowar time added : Only one fruit_bitmask is shown.
		printf("%d %f %f %f %f %lld %lld\nfruit_bitmask : %d \n", cur->d->son, cur->d->bounces[0], cur->d->bounces[1], cur->d->bounces[2],  cur->d->bounces[3], cur->d->times[0], cur->d->times[1], cur->d->fruit_bitmask[0]);
	  cur = cur->next;
    }
}
...
void Linkable::clone(Linkable *old)
{
	...
	int k = TOTAL_FRUIT_NUMBER / 32 ;
	for( int i = 0; i < k ; i++ )
	{
		fruit_bitmask[i] = old->fruit_bitmask[i];
	}
	//memcpy(fruit_bitmask, old->fruit_bitmask, sizeof(fruit_bitmask));
	// memcpy(times,old->times,RTIME_SIZE * sizeof(RTIME) );	
	/*	Sarowar time : end : 9	*/
}